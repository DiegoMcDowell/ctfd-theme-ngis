...
from CTFd.utils.team_icons import delete_team_icon, save_team_icon
...
@teams_namespace.route("/me/icon")
class TeamPrivateIcon(Resource):
    """Subida/borrado del icono del equipo propio. Solo el capitán."""
    ...
    def post(self):
        team = get_current_team()
        if team.captain_id != session["id"]:
            return _captain_only()
        return _update_icon(team, request.files.get("file"))
    ...
    def delete(self):
        ...
        if team.captain_id != session["id"]:
            return _captain_only()
        return _remove_icon(team)


@teams_namespace.route("/<int:team_id>/icon")
class TeamPublicIcon(Resource):
    """Subida/borrado del icono de cualquier equipo. Solo admins."""
...
def _update_icon(team, file_obj):
    if file_obj is None:
        return {"success": False, "errors": {"file": ["No file provided"]}}, 400
    try:
        icon = save_team_icon(team, file_obj)
    except ValueError as e:
        return {"success": False, "errors": {"file": [str(e)]}}, 400

    clear_team_session(team_id=team.id)
    clear_standings()
    return {"success": True, "data": {"icon": icon}}


def _remove_icon(team):
    delete_team_icon(team)
    clear_team_session(team_id=team.id)
    clear_standings()
    return {"success": True, "data": {"icon": None}}
